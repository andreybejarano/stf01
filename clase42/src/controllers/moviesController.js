...
const moviesController = {
    ...
    buscar: async (req, res) => {
        try {
            ...
            if (movies.length === 0) {
                const response = await fetch(`http://www.omdbapi.com/?apikey=7581f363&s=${req.body.titulo}&type=movie`);
                const moviesInOMDB = await response.json();
                if (moviesInOMDB.Error) {
                    return res.send([]);
                }
                for (let index = 0; index < moviesInOMDB.Search.length; index++) {
                    const response = await fetch(`http://www.omdbapi.com/?apikey=7581f363&i=${moviesInOMDB.Search[index].imdbID}`);
                    const data = await response.json();
                    const releaseDate = moment(data.Released).format('YYYY-MM-DD');
                    const length = data.Runtime.split(' ')[0];
